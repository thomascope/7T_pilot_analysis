function [sum_iterative_prediction_error, sum_iterative_sensory_error, this_correlation,heard_correct_word, amount_above_second, amount_above_average] = PINFA_blank_simplified(sigma_pred, threshold, sensory_detail)
...
% condition{5} = 'A heard, Neutral prior, Low sensory detail';
% condition_code(5,:) = [3,1];
% condition{6} = 'A heard, Neutral prior, High sensory detail';
% condition_code(6,:) = [3,2];
...
nIterations = 5; %How many iterations is reasonable in the timescale? (NB: Increment steps of 0.01)
...
prior_match_feature = wordToFeature_3d(prior_match_word,feature_mat_words);
prior_mismatch_feature = wordToFeature_3d(prior_mismatch_word,feature_mat_words);
prior_neutral_feature = wordToFeature_3d(prior_neutral_word,feature_mat_words);
...
input_noise = {};
for j = 1:nFeatures
input_noise{j} = abs(randn(size(prior_match_feature{j})));
input_noise{j} = input_noise{j}/max(max(input_noise{j})); %Scale 0-1
end
% 
% x = [-10:1:10];
% xdisplay = [-20 20];
PE_features = {};
PE_sensation = {};
sum_iterative_prediction_error = [];
sum_iterative_sensory_error = [];
posterior_features = {};
all_PE_features = [];
% For debug - how do parameter shifts change the output? Look for face validity and stability
% sigma_pred = sigma_pred/2 
% sensory_detail(1) = 0.2;
% visualise_this_outcome = 1; %To look at the settling process
visualise_this_feature = 0;

for i=1:length(condition)
%     if exist('visualise_this_outcome','var')&&visualise_this_outcome
%         figure(i)
%         set(gcf,'units','normalized','outerposition',[0 0 1 1])
%         sgtitle('Outcomes figure')
%     end
    if exist('visualise_this_feature','var')&&visualise_this_feature
        figure(i+10)
        set(gcf,'units','normalized','outerposition',[0 0 1 1])
        sgtitle('Feature probability figure')
    end
    ...
    for this_word = 1:nWords
        ...
        clear all_correlations posterior_features
        for j = 1:nFeatures
            % Bogacz 2017-like implementation - cf exercise 3
            %             this_input{j} = feature_mat_words{j}(this_word,:);
            this_input{j} = (feature_mat_words{j}(this_word,:)*this_sensory_detail)+input_noise{j}(this_word,:);
            %Now set minumum to 0 and sum to 1, as these are probabilities
            this_input{j} = max(this_input{j},0);
            this_input{j} = this_input{j}/sum(this_input{j});
            Sigma_input = 1-this_sensory_detail+0.01; % Assume a little noise even with 100% performance, to avoid divide by zero
            
            DT = 0.01; %Increment step
            clear phi error_p error_u
            phi(1,:) = this_prior_features{j}(this_word,:); % initialise the posterior
            error_p(1,:) = zeros(1,size(phi(1,:),2)); % initialise the prediction error of phoneme identity
            error_u(1,:) = zeros(1,size(phi(1,:),2)); % initialise the prediction error of sensory input
            
            for time = 2:nIterations/DT
                phi(time,:) = phi(time-1,:) + prior_update_weight*(-error_p(time-1,:) + error_u(time-1,:) .* (phi(time-1,:)));
                %Now set minumum to 0 and sum to 1, as these are probabilities
                phi(time,:) = max(phi(time,:),0);
                phi(time,:) = phi(time,:)/sum(phi(time,:));
                error_p(time,:) = error_p(time-1,:) + DT*(phi(time-1,:) - this_prior_features{j}(this_word,:) - ((sigma_pred^2)*error_p(time-1,:)));
                error_u(time,:) = error_u(time-1,:) + DT*(this_input{j} - phi(time-1,:) - (Sigma_input * error_u(time-1)));
            end
            
            
            if exist('visualise_this_feature','var')&&visualise_this_feature
                % To plot feature probability over time
                figure(i+10)
                subplot(nWords,nFeatures,(this_word-1)*nFeatures+j)
                plot([DT:DT:nIterations], phi , 'k');
                hold on
                plot([DT:DT:nIterations], error_p , 'r--');
                plot([DT:DT:nIterations], error_u , 'g:');
                ylim([0 1])
%                 xlabel ('Time ');
%                 ylabel ('Activity ');
                % To plot 
            ...
            sum_iterative_prediction_error(i,this_word,j) = sum(abs(error_p(end,:)),2);
            sum_iterative_sensory_error(i,this_word,j) = sum(abs(error_u(end,:)),2);
            posterior_features{j}(i,this_word,:) = phi(end,:);
            all_correlations(j,:) = corr(squeeze(posterior_features{j}(i,this_word,:)),feature_mat_words{j}');
        end
        across_feature_correlations = mean(all_correlations);
        [this_correlation(i,this_word),heard_word(i,this_word)] = max(across_feature_correlations);
        heard_correct_word(i,this_word) = double(heard_word(i,this_word)==this_word);
        amount_above_average(i,this_word) = max(across_feature_correlations)-((sum(across_feature_correlations)-max(across_feature_correlations))/length(across_feature_correlations));
        amount_above_second(i,this_word) = max(across_feature_correlations)-max(across_feature_correlations(across_feature_correlations<max(across_feature_correlations)));
    end
end
sum_iterative_prediction_error = sum(sum_iterative_prediction_error,3); % Sum across features
sum_iterative_sensory_error = sum(sum_iterative_sensory_error,3); % Sum across features

%pause % For debug
            
%             %Iterative loop - see how close we get in n iterations
%             %Old code - suboptimal behaviours because features not weighted
%             %equally
