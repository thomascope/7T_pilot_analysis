...
Mfilename = '_All_Pre'; %for concatenated runs the name will be: ['SubjInitials' 'Mfilename']

if nargin > 0, Ses{1} = SesIn; end
if nargin == 0,
    analysismode = input('Do you want to process by type (1) or by subject (2)?');
    if analysismode == 1
        Patfiles = ls([fpath 'Patients\Vocode*Pre*']);
        Confiles = ls([fpath 'Controls\Vocode*Pre*']);
...
if analysismode == 1
    for S = 1:size(PatSes,2),
        ...
        saveas(gcf,[DIRS.dataFigs 'Vocode Report Patient ' Patinits(S,:) ' Pre.jpeg'])
    ...
    for S = 1:size(ConSes,2),
        ...
        saveas(gcf,[DIRS.dataFigs 'Vocode Report Control ' Coninits(S,:) ' Pre.jpeg'])
...
if multi,
    if analysismode == 1
        ...
        figure
        set(gcf,'position',[100,100,1200,800])
        barweb(mean_patient_meansarray,ste_patient_meansarray,[],patdat{1}.vocode_channels,['Prescan Vocode Report Percent Correct for All Patients'],[],'Percent Correct',[],[],{'Base','Onset','Offset','Mismatch'}) ;
        legend('2 neighbours','Onset neighbour','Offset neighbour','No neighbours','location','NorthWest');
        
        saveas(gcf,[DIRS.dataFigs 'Vocode Report All Patients Pre.jpeg'])
        ...
        figure
        set(gcf,'position',[100,100,1200,800])
        barweb(mean_control_meansarray,ste_control_meansarray,[],condat{1}.vocode_channels,['Prescan Vocode Report Percent Correct for All Controls'],[],'Percent Correct',[],[],{'Base','Onset','Offset','Mismatch'}) ;
        legend('2 neighbours','Onset neighbour','Offset neighbour','No neighbours','location','NorthWest');
        
        saveas(gcf,[DIRS.dataFigs 'Vocode Report All Controls Pre.jpeg'])
