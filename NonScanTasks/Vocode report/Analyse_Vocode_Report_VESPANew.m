...
Mfilename = '_All_New'; %for concatenated runs the name will be: ['SubjInitials' 'Mfilename']

if nargin > 0, Ses{1} = SesIn; end
if nargin == 0,
    analysismode = input('Do you want to process by type (1) or by subject (2)?');
    if analysismode == 1
        Patfiles = ls([fpath 'Patients_New\Vocode*New*']);
        Confiles = ls([fpath 'Controls_New\Vocode*New*']);
        startnumber = strfind(Patfiles(1,:),'P7P');
        Patinits = unique(Patfiles(:,startnumber:startnumber+4),'rows'); %Assumes all 5 letter ID codes and 2 digit ages.
        ...
        for I = 1:size(Patfiles,1)
            PatSes{I} = ['Patients_New\' Patfiles(I,:)];
        end
        for I = 1:size(Confiles,1)
            ConSes{I} = ['Controls_New\' Confiles(I,:)];
        end
...
if analysismode == 1
    for S = 1:size(PatSes,2),
        ...
        patdat.trialarray = zeros(length(patdat.correct),3); % array of correct, target, difficulty
        patdat.trialarray(:,1) = patdat.correct;
        patdat.trialarray(:,2) = patdat.target_list; %Here all 2 neighbours
        ...
        for i = 1:size(patdat.trialarray,1)
            if patdat.trialarray(i,2)==1 && patdat.trialarray(i,3)==1
                base4 =[base4, patdat.trialarray(i,1)];
            elseif patdat.trialarray(i,2)==2 && patdat.trialarray(i,3)==1
        ...
        legend('2 neighbours','Onset neighbour','Offset neighbour','No neighbours','location','NorthWest');
        
        patdat.meansarray = meansarray;
        patdat.errorsarray = errorsarray;
        
        %save individual "patdat" structures
        save([fpath 'beh_' PatSes{S}(20:end)],'patdat');
        saveas(gcf,[DIRS.dataFigs 'Vocode Report Patient ' Patinits(S,:) ' New.jpeg'])
    ...
    for S = 1:size(ConSes,2),
        ...
        Condat.trialarray = zeros(length(Condat.correct),3); % array of correct, target, difficulty
        ...
        for i = 1:size(Condat.trialarray,1)
            if Condat.trialarray(i,2)==1 && Condat.trialarray(i,3)==1
                base4 =[base4, Condat.trialarray(i,1)];
            elseif Condat.trialarray(i,2)==2 && Condat.trialarray(i,3)==1
        ...
        Condat.errorsarray = errorsarray;
        
        %save individual "Condat" structures
        save([fpath 'beh_' ConSes{S}(20:end)],'Condat');
        saveas(gcf,[DIRS.dataFigs 'Vocode Report Control ' Coninits(S,:) ' New.jpeg'])
...
if multi,
    if analysismode == 1
        ...
        for i = 1:length(patdat)
            all_patient_meansarray(:,:,i) = patdat{i}.meansarray;
        end
        mean_patient_meansarray = mean(all_patient_meansarray,3);
        ste_patient_meansarray = std(all_patient_meansarray,0,3)/sqrt(size(all_patient_meansarray,3));

        figure
        set(gcf,'position',[100,100,1200,800])
        barweb(mean_patient_meansarray,ste_patient_meansarray,[],patdat{1}.vocode_channels,['Newscan Vocode Report Percent Correct for All Patients'],[],'Percent Correct',[],[],{'Base','Onset','Offset','Mismatch'}) ;
        legend('2 neighbours','Onset neighbour','Offset neighbour','No neighbours','location','NorthWest');
        
        saveas(gcf,[DIRS.dataFigs 'Vocode Report All Patients New.jpeg'])
        
        all_control_meansarray = [];
        for i = 1:length(condat)
            all_control_meansarray(:,:,i) = condat{i}.meansarray;
        end
        mean_control_meansarray = mean(all_control_meansarray,3);
        ste_control_meansarray = std(all_control_meansarray,0,3)/sqrt(size(all_control_meansarray,3));

        figure
        set(gcf,'position',[100,100,1200,800])
        barweb(mean_control_meansarray,ste_control_meansarray,[],condat{1}.vocode_channels,['Newscan Vocode Report Percent Correct for All Controls'],[],'Percent Correct',[],[],{'Base','Onset','Offset','Mismatch'}) ;
        legend('2 neighbours','Onset neighbour','Offset neighbour','No neighbours','location','NorthWest');
        
        saveas(gcf,[DIRS.dataFigs 'Vocode Report All Controls New.jpeg'])
