...
if ~segmented
nrun = length(mrilist);
jobfile = {'./vbm_scripts/VBM_batch_segment.m'};
...
nrun = 1;
jobfile = {'./vbm_scripts/VBM_batch_TIV.m'};
jobs = repmat(jobfile, 1, nrun);
inputs = cell(2, nrun);
...
jobfile = {'./vbm_scripts/VBM_batch_dartel.m'};
jobs = repmat(jobfile, 1, nrun);
inputs = cell(2,1);
...
jobfile = {'./vbm_scripts/VBM_batch_factorial_TIV_age_singlesubj.m'};
jobs = repmat(jobfile, 1, 1);
inputs = cell(6, nrun);
